...
namespace TransportCompany.DAO
{
    public class TransportDAO
    {
        ...
        public bool updateType(string oldName, Transport newTransport)
        {
            try
            {
                string query = String.Format("UPDATE Transport SET license_plate = '{0}', driver_name = '{1}', model_name = '{2}' WHERE license_plate = '{3}' ", newTransport.LicensePlate, newTransport.DriverName, newTransport.ModelName, oldName);
                connect.executeNonQuery(query);
                return true;
            }
            catch (Exception ex)
